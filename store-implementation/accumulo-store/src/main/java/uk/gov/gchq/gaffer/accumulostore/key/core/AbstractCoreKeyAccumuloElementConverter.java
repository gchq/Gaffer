        ...
        if (bytes != null && bytes.length != 0) {
            ...
            while (propertyNames.hasNext() && delimiterPosition < arrayLength) {
                ...
                try {
                    ...
                    if (null != serialiser) {
                        final int numBytesForLength = CompactRawSerialisationUtils.decodeVIntSize(bytes[delimiterPosition]);
                        long currentPropLength = getCurrentPropLength(bytes, delimiterPosition, numBytesForLength);
                        Object deserialisedObject = getDeserialisedObjectOrEmpty(serialiser, bytes, delimiterPosition + numBytesForLength, delimiterPosition + numBytesForLength + (int) currentPropLength);
                        properties.put(propertyName, deserialisedObject);
                        delimiterPosition += numBytesForLength + currentPropLength;
                    }
                } catch (final SerialisationException e) {
