 ...
 * into a {@link org.apache.hadoop.util.bloom.BloomFilter}. This is passed to the iterators to filter out all edges that are definitely not between
 * elements of the set. A secondary check is done within this class to check that the edge is definitely between
 * elements of the set (this defeats any false positives, i.e. edges that passed the {@link org.apache.hadoop.util.bloom.BloomFilter} check
 ...
 * When the first batch is queried for, a {@link org.apache.hadoop.util.bloom.BloomFilter} of the first batch is created and passed to the iterators.
 * This filters out all edges that are definitely not between elements of the first batch. When the second batch is
 * queried for, the same {@link org.apache.hadoop.util.bloom.BloomFilter} has the second batch added to it. This is passed to the iterators, which
 ...
 * client-side {@link org.apache.hadoop.util.bloom.BloomFilter} to further reduce the chances of false positives making it to the user.
 */
