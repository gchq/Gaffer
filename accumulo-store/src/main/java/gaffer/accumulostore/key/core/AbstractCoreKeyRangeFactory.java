...
public abstract class AbstractCoreKeyRangeFactory implements RangeFactory {
    ...
    public <T extends GetOperation<?, ?>> List<Range> getRange(final ElementSeed elementSeed, final T operation) throws RangeFactoryException {
        if (elementSeed instanceof EntitySeed) {
            ...
            if (IncludeEdgeType.ALL == operation.getIncludeEdges()
                ...
                // Get Edges with the given EdgeSeed - This is applicable for EQUALS and RELATED seed matching.
    ...
    protected abstract <T extends GetOperation<?, ?>> Key getKeyFromEdgeSeed(final EdgeSeed seed, final T operation, final boolean endKey) throws RangeFactoryException;

    protected abstract <T extends GetOperation<?, ?>> List<Range> getRange(final Object vertex, final T operation, final IncludeEdgeType includeEdgesParam) throws RangeFactoryException;
}
