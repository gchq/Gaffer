...
public class MiniAccumuloStore extends AccumuloStore {
    ...
    private void ensureUserExists(final MiniAccumuloCluster mac) throws AccumuloSecurityException, AccumuloException {
        final String userName = getProperties().getUser();
        final String rootPassword = getProperties().get(ROOT_PASSWORD_PROPERTY, ROOT_PASSWORD_DEFAULT);

        final String zooKeepers = mac.getZooKeepers();
        final String instanceName = mac.getInstanceName();
        final ClientConfiguration clientConfiguration = mac.getClientConfig();
        final Set<Pair<ServerType, Integer>> debugPorts = mac.getDebugPorts();
        try {
            mac.stop();
            mac.start();
        ...
        // Ensure user exists
        synchronized (this) {
            final Set<String> currentUsers = mac.getConnector(ROOT_USER, rootPassword).securityOperations().listLocalUsers();
        ...
        // Add Auths
        final String auths = getProperties().get(VISIBILITIES_PROPERTY);
        if (auths != null) {
            mac.getConnector(ROOT_USER, rootPassword).securityOperations()
    ...
    private void createCluster() throws IOException, InterruptedException {
        final String providedDirectory = getProperties().get(ACCUMULO_DIRECTORY_PROPERTY);
        ...
        final String rootUserPassword = getProperties().get(ROOT_PASSWORD_PROPERTY, ROOT_PASSWORD_DEFAULT);

        final MiniAccumuloConfig config = new MiniAccumuloConfig(getAccumuloDirectory(), rootUserPassword);

        final String[] zookeepers = getProperties().getZookeepers().split(":");
