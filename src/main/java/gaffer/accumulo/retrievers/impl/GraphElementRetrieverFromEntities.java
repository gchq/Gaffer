 ...
 * are provided as {@link GraphElement} which means that the user does not have
 ...
 */
public class GraphElementRetrieverFromEntities implements CloseableIterable<GraphElement> {
	...
	// Iterator
	private GraphElementIterator graphElementIterator = null;
	
	public GraphElementRetrieverFromEntities(Connector connector, Authorizations auths, String tableName,
	...
	public Iterator<GraphElement> iterator() {
		...
		graphElementIterator = new GraphElementIterator();
		return graphElementIterator;
	}

	@Override
	public void close() {
		if (graphElementIterator != null) {
			graphElementIterator.close();
		}
	}
	
	private class GraphElementIterator implements Iterator<GraphElement> {
		...
		public GraphElement next() {
			...
			try {
				...
				return transformed.getGraphElement();
			} catch (IOException e) {
				return null;
			}
		}
