...
public class WriteDataToContainerTest {
    ...
    public void shouldRetrieveInputStream() throws InterruptedException {
        // Given
        Thread serverThread = setupTestServer(ScriptTestConstants.TEST_SERVER_PORT_1);
        Socket testSocket = null;
        for (int i = 0; i < 3; i++) {
            try {
                testSocket = new Socket(ScriptTestConstants.LOCALHOST, ScriptTestConstants.TEST_SERVER_PORT_1);
                break;
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public void shouldSendAndReceiveDataToSocket() throws InterruptedException {
        // Given
        Thread serverThread = setupTestServer(ScriptTestConstants.TEST_SERVER_PORT_2);
        ...
        for (int i = 0; i < 3; i++) {
            try {
                testSocket = new Socket(ScriptTestConstants.LOCALHOST, ScriptTestConstants.TEST_SERVER_PORT_2);
                break;
            } catch (IOException e) {
                e.printStackTrace();
            }
