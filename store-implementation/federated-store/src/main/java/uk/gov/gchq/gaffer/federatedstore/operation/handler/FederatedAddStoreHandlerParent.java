...
/**
 * A handler for operations that addStore to the FederatedStore.
 *
 * @see OperationHandler
 * @see FederatedStore
 * @see Store
 */
public abstract class FederatedAddStoreHandlerParent<OP extends AddGraph> implements OperationHandler<OP> {
    ...
    public Void doOperation(final OP operation, final Context context, final Store store) throws OperationException {
        ...
        final Store storeToAdd = store;

        try {
            ((FederatedStore) store).addStores(operation.getGraphAuths(),
                    context.getUser().getUserId(), operation.getIsPublic(),
                    operation.isDisabledByDefault(), storeToAdd);
        ...
        addGenericHandler((FederatedStore) store, storeToAdd);

        return null;
    }

    protected void addGenericHandler(final FederatedStore store,
                                     final Store storeToAdd) {
        for (final Class<? extends Operation> supportedOperation :
                storeToAdd.getSupportedOperations()) {
            ...
            if (Output.class.isAssignableFrom(supportedOperation) && !store.isSupported(supportedOperation)) {
                ...
                if (CloseableIterable.class.equals(outputClass)) {
                    store.getConfig().addOperationHandler((Class) supportedOutputOperation,
