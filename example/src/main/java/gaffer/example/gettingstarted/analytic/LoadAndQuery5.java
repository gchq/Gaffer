...
public class LoadAndQuery5 extends LoadAndQuery {
    ...
    public Iterable<Edge> run() throws OperationException {
        ...
        final GetRelatedEdges<EntitySeed> getPrivateAggregatedRelatedEdges = new GetRelatedEdges.Builder<EntitySeed>()
                ...
                .summarise(true)
                .build();
        final Iterable<Edge> privateAggregatedResults = graph.execute(getPrivateAggregatedRelatedEdges, privateUser);
        for (Element e : privateAggregatedResults) {
            log("GET_PRIVATE_AGGREGATED_RELATED_EDGES_RESULT", e.toString());
        }
