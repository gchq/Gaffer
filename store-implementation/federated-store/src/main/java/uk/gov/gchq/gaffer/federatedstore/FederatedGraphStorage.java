...
public class FederatedGraphStorage {
    ...
    protected Map<String, Object> getAllGraphsAndAccess(final User user, final List<String> graphIds, final List<String> excludedGraphIds) {
        return getAllGraphsAndAccess(graphIds, excludedGraphIds, access -> access != null && access.hasReadAccess(user));
    }

    protected Map<String, Object> getAllGraphsAndAccess(final User user, final List<String> whiteListGraphIds, final List<String> excludedGraphIds, final String adminAuth) {
        return getAllGraphsAndAccess(whiteListGraphIds, excludedGraphIds, access -> access != null && access.hasReadAccess(user, adminAuth));
    }

    private Map<String, Object> getAllGraphsAndAccess(final List<String> whiteListGraphIds, final List<String> excludedGraphIds, final Predicate<FederatedAccess> accessPredicate) {
        return StreamSupport.stream(federatedStoreCache.getAllGraphIds().spliterator(), false)
                .map(federatedStoreCache::getFromCache)
                ...
                .filter(pair -> {
                    ...
                    final boolean isExcluded = excludedGraphIds != null && excludedGraphIds.contains(pair.getFirst().getGraphId());
                    final boolean isAllGraphIdsRequired = isNull(whiteListGraphIds) || whiteListGraphIds.isEmpty();
                    return (!isExcluded && isGraphIdRequested) || (!isExcluded && isAllGraphIdsRequired);
                })
