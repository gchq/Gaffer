...
import gaffer.accumulo.retrievers.impl.GraphElementWithStatisticsRetrieverFromRanges;
...
/**
 * Unit test for {@link GraphElementWithStatisticsRetrieverFromRanges}. In particular tests that if more ranges
 ...
 */
public class TestGraphElementWithStatisticsFromRangesRetriever {
	...
	public void test() {
		try {
			...
			for (int i = 0; i < 1000; i++) {
				...
				typeValueRanges.add(new TypeValueRange("customer", s, "customer", s + "Z"));
			}
			...
			GraphElementWithStatisticsRetrieverFromRanges retriever = new GraphElementWithStatisticsRetrieverFromRanges(conn, new Authorizations(visibilityString), tableName, 10000, 2,
					true, predicate, null, null, true, true, typeValueRanges);
			int count = 0;
			...
			retriever = new GraphElementWithStatisticsRetrieverFromRanges(conn, new Authorizations(visibilityString), tableName, 100, 2,
					true, predicate, null, null, true, true, typeValueRanges);
			count = 0;
			for (@SuppressWarnings("unused") GraphElementWithStatistics elementWithStatistics : retriever) {
				count++;
			...
			retriever = new GraphElementWithStatisticsRetrieverFromRanges(conn, new Authorizations(visibilityString), tableName, 1000, 2,
					true, predicate, null, null, true, true, typeValueRanges);
			count = 0;
			for (@SuppressWarnings("unused") GraphElementWithStatistics elementWithStatistics : retriever) {
				count++;
	...
	private static AccumuloBackedGraph setupGraph(Instance instance, Connector conn, String tableName, int numEntries) {
		...
		try {
			...
			for (int i = 0; i < numEntries; i++) {
				...
				Edge edge = new Edge("customer", s, "product", "B", "purchase", "instore", true, visibilityString, sevenDaysBefore, sixDaysBefore);
