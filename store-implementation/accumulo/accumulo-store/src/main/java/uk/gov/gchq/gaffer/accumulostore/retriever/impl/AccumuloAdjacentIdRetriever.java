...
public class AccumuloAdjacentIdRetriever extends AccumuloRetriever<GetAdjacentIds, EntityId> {
    ...
    private Set<String> getGroupsWithTransforms(final View view) {
        ...
        try {
            chainedIterable = new ChainedIterable<Entry<String, ViewElementDefinition>>(view.getEntities().entrySet(), view.getEdges().entrySet());
            for (final Entry<String, ViewElementDefinition> entry : chainedIterable) {
