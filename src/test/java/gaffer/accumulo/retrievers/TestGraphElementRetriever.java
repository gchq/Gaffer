...
import gaffer.accumulo.retrievers.impl.GraphElementRetrieverFromEntities;
...
/**
 * Unit test for {@link GraphElementRetrieverFromEntities}. In particular tests that if more entities
 ...
 */
public class TestGraphElementRetriever {
	...
	public void test() {
		try {
			...
			CloseableIterable<GraphElement> retriever = new GraphElementRetrieverFromEntities(conn, new Authorizations(visibilityString), tableName, 10000, 2,
					true, predicate, null, null, true, true, typeValues);
			int count = 0;
			for (@SuppressWarnings("unused") GraphElement element : retriever) {
				count++;
			}
			...
			retriever = new GraphElementRetrieverFromEntities(conn, new Authorizations(visibilityString), tableName, 100, 2,
					true, predicate, null, null, true, true, typeValues);
			count = 0;
			for (@SuppressWarnings("unused") GraphElement element : retriever) {
				count++;
			...
			retriever = new GraphElementRetrieverFromEntities(conn, new Authorizations(visibilityString), tableName, 1000, 2,
					true, predicate, null, null, true, true, typeValues);
			count = 0;
			for (@SuppressWarnings("unused") GraphElement element : retriever) {
				count++;
